...
install_java() {
    ...
    if apt-get install -y openjdk-21-jdk; then
        ...
        if command_exists java; then
            ...
            JAVA_HOME_PATH=$(update-alternatives --list java 2>/dev/null | head -n 1 | sed 's|/bin/java||')
            ...
            echo "JDK installation completed but java command not found"
...
install_maven() {
    ...
    if apt-get install -y maven; then
        ...
        if command_exists mvn; then
            ...
            echo "Maven installation completed but mvn command not found"
...
install_git() {
    ...
    if apt-get install -y git; then
        ...
        if command_exists git; then
            ...
            echo "Git installation completed but git command not found"
...
clone_stellar_project() {
    ...
    if git clone https://asadrazamahmood@bitbucket.org/stellar2/stellar-sample-project.git "$STELLAR_PROJECT_DIR"; then
