...
check_jdk_installed() {
    if type -p java >/dev/null 2>&1; then
        ...
        if [[ "$JAVA_VERSION" == "21"* ]]; then
            echo "JDK 21 is already installed (version $JAVA_VERSION)"
            return 0
        else
            echo "Found Java version $JAVA_VERSION but JDK 21 is required"
            return 1
        fi
    else
...
## Function to clone Stellar sample project
clone_stellar_project() {
    PROJECT_URL="https://asadrazamahmood@bitbucket.org/stellar2/stellar-sample-project.git"
    CLONE_DIR="$HOME/stellar-sample-project"
    
    echo "Cloning Stellar sample project..."
    
    # Agar project already exists to overwrite karo
    if [ -d "$CLONE_DIR" ]; then
        echo "Project directory already exists. Overwriting with latest version..."
        cd "$CLONE_DIR"

        # Stash local changes agar koi hain
        ...
        if [ $? -eq 0 ]; then
            echo "Project successfully updated with latest changes!"
        else
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "Error: Failed to clone/update Stellar project"
...
echo ""
echo "=== JDK 21 INSTALLATION ==="
detect_architecture

## JDK 21 Installation (only if needed)
...
echo "Stellar sample project location: $HOME/stellar-sample-project"
echo ""
echo "To run the project:"
echo "cd $HOME/stellar-sample-project"
echo "mvn clean test"
